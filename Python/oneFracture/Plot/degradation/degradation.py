...
plt.plot(timeStep, exp_prob, 'r-', label='Exp pdf',  linewidth=3)
plt.plot(Time[::10], numOfLivePart[::10]/numOfLivePart.sum(), 'o', markerfacecolor='none', markeredgecolor='blue', markersize='8', label='Direct simulation')
# plt.title("Live particle distribution in time")
plt.xscale('log')
plt.yscale('log')
plt.xlim(0, t+0.05*t)
# plt.ylim(min(numOfLivePart[:-3]/numOfLivePart.sum()), max(numOfLivePart/numOfLivePart.sum())+0.1*max(numOfLivePart/numOfLivePart.sum()))
plt.ylim(1e-8, max(numOfLivePart/numOfLivePart.sum())+0.1*max(numOfLivePart/numOfLivePart.sum()))
