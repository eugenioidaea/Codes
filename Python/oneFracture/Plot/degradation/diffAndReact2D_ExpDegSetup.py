...
def degradation_dist(num_steps, kDecay, num_particles):
    ...
    exp_prob = kDecay*np.exp(-kDecay*timeStep)
    exp_prob = exp_prob/exp_prob.sum()
    survivalTimeDist = np.random.choice(timeStep, size=num_particles, p=exp_prob)
...
if save.upper()=="Y":
    ...
    np.savez('onlyDecayTau400.npz', **variablesToSave)
    if variablesToSave:
        print("\n RESULTS SAVED")
else:
    print("\n RESULTS NOT SAVED.")